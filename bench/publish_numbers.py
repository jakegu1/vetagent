...
TARGETS = [
    ...
    # The venue texts of 2026-09-19 -- X as posted, r/ethdev and the MCP Discord for the owner
    # to post. One named pattern per copy: a target reads only its first match, and a value
    # claimed elsewhere in the file looks guarded exactly as long as it is right (the five
    # fp_pct copies above). The retired drafts' targets went with their text.
    ("docs/EXPERIMENT_C.md", r"Of (\d+) contracts a held-out scanner calls adversarial, my", "adversarial_n"),
    ("docs/EXPERIMENT_C.md", r"for AI agents rates (\d+) high risk \(", "adversarial_high_n"),
    ("docs/EXPERIMENT_C.md", r"for AI agents rates \d+ high risk \(([\d.]+)%\)", "adversarial_high_pct"),
    ("docs/EXPERIMENT_C.md", r"(\d+) of those pools hold under \$1 of liquidity", "adversarial_sub_dollar_n"),
    ("docs/EXPERIMENT_C.md", r"contract signals alone: ([\d.]+)%", "adversarial_high_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"my own open-source pre-trade token-risk check: ([\d.]+)% of adversarial", "adversarial_high_pct"),
    ("docs/EXPERIMENT_C.md", r"rated high, ([\d.]+)% on contract signals alone \(n=\d+\)", "adversarial_high_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"on contract signals alone \(n=(\d+)\)\. The harness", "adversarial_n"),
    ("docs/EXPERIMENT_C.md", r"On (\d+) contracts a held-out oracle labels adversarial", "adversarial_n"),
    ("docs/EXPERIMENT_C.md", r"oracle labels adversarial, it rates ([\d.]+)% \(\d+ of \d+\) high", "adversarial_high_pct"),
    ("docs/EXPERIMENT_C.md", r"oracle labels adversarial, it rates [\d.]+% \((\d+) of \d+\) high", "adversarial_high_n"),
    ("docs/EXPERIMENT_C.md", r"impersonation and the rest\) and it is ([\d.]+)% high", "adversarial_high_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"% high, with ([\d.]+)% rated \*low\*", "adversarial_low_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"(\d+) of those 17 hold under a dollar of liquidity, so the full column is substantially detecting", "adversarial_sub_dollar_n"),
    ("docs/EXPERIMENT_C.md", r"(\d+) of the 17 are on Base", "adversarial_base_n"),
    ("docs/EXPERIMENT_C.md", r"(\d+) tokens on Ethereum, BSC and Base \(", "n"),
    ("docs/EXPERIMENT_C.md", r"tokens on Ethereum, BSC and Base \(([\d.]+)% Base\)", "base_share_pct"),
    ("docs/EXPERIMENT_C.md", r"Of (\d+) tokens that actually died, ", "dead_n"),
    ("docs/EXPERIMENT_C.md", r"actually died, ([\d.]+)% \(\d+ of \d+\) are rated high", "dead_high_pct"),
    ("docs/EXPERIMENT_C.md", r"actually died, [\d.]+% \((\d+) of \d+\) are rated high", "dead_high_n"),
    ("docs/EXPERIMENT_C.md", r"are rated high\. ([\d.]+)% are not rated low", "dead_not_low_pct"),
    ("docs/EXPERIMENT_C.md", r"are not rated low, but that falls to ([\d.]+)% on contract signals only", "dead_not_low_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"False positives: ([\d.]+)% \(\d+ of \d+\) against market outcome", "fp_pct"),
    ("docs/EXPERIMENT_C.md", r"False positives: [\d.]+% \((\d+) of \d+\) against", "fp_n"),
    ("docs/EXPERIMENT_C.md", r"False positives: [\d.]+% \(\d+ of (\d+)\) against", "healthy_n"),
    ("docs/EXPERIMENT_C.md", r"against market outcome, ([\d.]+)% against the held-out contract oracle", "goplus_fp_pct"),
    ("docs/EXPERIMENT_C.md", r"The healthy control's median pool is \$([\d,]*\d)", "alive_median_liq"),
    ("docs/EXPERIMENT_C.md", r"and all (\d+) are among the \d+ thin \(under", "alive_thin_high_n"),
    ("docs/EXPERIMENT_C.md", r"are among the (\d+) thin \(under \$10k\)", "alive_thin_n"),
    ("docs/EXPERIMENT_C.md", r"- ([\d.]+)% \(\d+ of \d+\) of liquid \(\$100k\+\) tokens", "false_block_pct"),
    ("docs/EXPERIMENT_C.md", r"- [\d.]+% \((\d+) of \d+\) of liquid \(\$100k\+\) tokens", "false_block_n"),
    ("docs/EXPERIMENT_C.md", r"- [\d.]+% \(\d+ of (\d+)\) of liquid \(\$100k\+\) tokens", "false_block_of"),
    ("docs/EXPERIMENT_C.md", r"likely refuse; another (\d+) come back `unknown`", "false_block_unknown_n"),
    ("docs/EXPERIMENT_C.md", r"moved this from \d+ to (\d+); I chose it", "false_block_n"),
    ("docs/EXPERIMENT_C.md", r"I chose it with these same (\d+) tokens in view, so treat", "n"),
    ("docs/EXPERIMENT_C.md", r"- ([\d.]+)% \(\d+ of \d+\) of benchmark answers are `unknown`", "unknown_pct"),
    ...
    ("docs/EXPERIMENT_C.md", r"labels adversarial, rated high: \*\*([\d.]+)%\*\* \(\d+ of \d+\)", "adversarial_high_pct"),
    ("docs/EXPERIMENT_C.md", r"labels adversarial, rated high: \*\*[\d.]+%\*\* \((\d+) of \d+\)", "adversarial_high_n"),
    ("docs/EXPERIMENT_C.md", r"labels adversarial, rated high: \*\*[\d.]+%\*\* \(\d+ of (\d+)\)", "adversarial_n"),
    ("docs/EXPERIMENT_C.md", r"; contract signals only: \*\*([\d.]+)%\*\*", "adversarial_high_ablated_pct"),
    ("docs/EXPERIMENT_C.md", r"(\d+) of the 17 hold under \$1 of liquidity", "adversarial_sub_dollar_n"),
    ("docs/EXPERIMENT_C.md", r"and (\d+) of the \d+ highs vanish", "adversarial_high_vanish_n"),
    ("docs/EXPERIMENT_C.md", r"and \d+ of the (\d+) highs vanish", "adversarial_high_n"),
    ("docs/EXPERIMENT_C.md", r"Confirmed-dead tokens rated high: \*\*([\d.]+)%\*\* \(\d+ of \d+\)", "dead_high_pct"),
    ("docs/EXPERIMENT_C.md", r"Confirmed-dead tokens rated high: \*\*[\d.]+%\*\* \((\d+) of \d+\)", "dead_high_n"),
    ("docs/EXPERIMENT_C.md", r"Confirmed-dead tokens rated high: \*\*[\d.]+%\*\* \(\d+ of (\d+)\)", "dead_n"),
    ("docs/EXPERIMENT_C.md", r"merely centralised, rated medium or high: \*\*([\d.]+)%\*\*", "false_block_pct"),
    ("docs/EXPERIMENT_C.md", r"merely centralised, rated medium or high: \*\*[\d.]+%\*\* \((\d+) of \d+\)", "false_block_n"),
    ("docs/EXPERIMENT_C.md", r"merely centralised, rated medium or high: \*\*[\d.]+%\*\* \(\d+ of (\d+)\)", "false_block_of"),
    ("docs/EXPERIMENT_C.md", r"\), plus (\d+) `unknown`", "false_block_unknown_n"),
    ...
    ("docs/EXPERIMENT_C.md", r"Healthy tokens rated high: \*\*[\d.]+%\*\* \((\d+) of \d+\) against", "fp_n"),
    ("docs/EXPERIMENT_C.md", r"Healthy tokens rated high: \*\*[\d.]+%\*\* \(\d+ of (\d+)\) against", "healthy_n"),
    ("docs/EXPERIMENT_C.md", r"all (\d+) among the \d+ where it saw under", "alive_thin_high_n"),
    ("docs/EXPERIMENT_C.md", r"all \d+ among the (\d+) where it saw under", "alive_thin_n"),
    ("docs/EXPERIMENT_C.md", r"where it saw under \$10k; ([\d.]+)% against the held-out scanner", "goplus_fp_pct"),
...
def figures():
    ...
    out = {
        ...
        # The venue texts of 2026-09-19 say what the false-block row leaves out: an agent that
        # follows the tool also declines every `unknown` in the same population.
        "false_block_unknown_n": "%d" % len([r for r in _false_block_population(rows)
                                             if r["verdict"] == "unknown"]),
        "adversarial_base_n": "%d" % len([r for r in unsafe_rows if r.get("chain") == "base"]),
        "adversarial_high_vanish_n": "%d" % len([r for r in unsafe_rows if r.get("verdict") == "high"
                                                 and r.get("verdict_ablated") != "high"]),
