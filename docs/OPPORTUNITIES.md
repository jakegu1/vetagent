...
### O6 · Holders who actually failed to sell, as a label the engine cannot see · **CLOSED 2026-09-18: run the measurement, change no label before 10-16**

Reviewed at its gate. The data trigger was met well before it: 630 answered sellability rows against the 500 required, 543 of them carrying `holderAnalysis`, 34 with `failed` above zero. Decision: the measurement runs, as agent work -- how often tokens whose sampled holders could not sell later die, against the same tokens' `dead`/`alive` outcome labels -- and no label or published number changes before the 2026-10-16 gate. The engine has read the field as disclosure since W34 (2026-09-15); whether it may change a verdict is the owner's W44. The original entry follows.
