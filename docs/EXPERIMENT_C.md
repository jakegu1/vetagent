...
deliberately does not do: score dormant owner powers.

The bytecode scan looks for four powers an owner could use to close the exit. **On a
contract it has never seen, it finds 50.0% of the ones an independent oracle asserts** —
31.6% of mutable taxes, 47.4% of pause switches, 53.2% of mint functions, 63.2% of
blacklists. That is the number worth quoting, because every contract you ask about is one
it has not seen.

Scored on the 559 contracts the selector list was *mined from*, the same scan reads 62.5%,
and the tax figure reads 89.5%. Those were the published figures for three days until an
audit pointed out they are in-sample: 193 of the 285 selectors occur in exactly one corpus
contract, so much of that 89.5% is memory of a specific token rather than a rule about how
Solidity names things. Both numbers are in `bench/owner_powers.json`, generated by one
command, and the gap between them is the most useful thing in the file.
