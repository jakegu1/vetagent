...
| # | Item | Why it matters | Verify | State |
|---|---|---|---|---|
| W5 | A second, independent sell-simulation source | W4's real fix. One vendor's absence should not be decisive, and the simulator covers three chains of the seven we advertise as hints | **Blocked** on a credential, not on engineering. Probed 2026-09-06: staysafu unreachable (SSL), quickintel 401, tokensniffer 401, de.fi public endpoint 404. Every candidate needs a paid key — this is a W9-shaped item that belongs to whoever holds the budget | Blocked |
...
| W19 | Measure whether the pool we pick is the one that is wrong | OPPORTUNITIES O4. 214 of 693 tokens (30.9%) have two pools disagreeing on price by >100x, 31 of them (4.5%) with both pools doing $50k/day. MATIC's own fixture spans $0.0882 to $1,345,090 on Ethereum. Firing rate is known; accuracy is not, and the price is the primary number this tool reports | For the disagreeing tokens, GeckoTerminal OHLCV price per pool, and a count of how often the deepest pool is the outlier rather than the dissenter. One pass over cached data, no new source | Open |
...
| W1 | Detect dormant owner powers from bytecode | **Done R13, and its R12 conclusion withdrawn.** Keccak-256 implemented so selectors are computed rather than remembered. The R12 measurement said the powers do not discriminate — but checked against the labelling oracle's own flags over 120 contracts it says hold at least one, the scan finds **31%** of them (5% for a mutable tax, 20% blacklist, 25% pause, 38% mint). The instrument is two-thirds blind, so that conclusion is not established either way. Disclosure ships, a hit is real, and silence now says so explicitly rather than reading as a clean contract |
