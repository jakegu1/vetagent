...
| W10 | List on mcp.so, the Claude plugin directory, awesome-mcp-servers, mcpservers.org | Distribution is 5/15 and every channel needs an account I cannot create. **Glama and Smithery are done (2026-09-08):** Glama says "Ownership verified" and health-checks the endpoint hourly, Smithery says "Published Sep 7, 2026" with the domain TXT and backlink both in place. Four channels left, and awesome-mcp-servers expects a listing to exist, which now it does | `CHANNELS` in `bench/scorecard.py`, updated only by someone who went and looked | Open |
