...
|---|---|---|---|---|
| W1 | Detect dormant owner powers (pause / blacklist / switchable tax) by reading 4-byte selectors out of contract bytecode | The audit's adversarial `low`: a not-yet-activated rug passes every check today. These are the fields B2 holds out, so bytecode is the route that keeps the oracle intact | Measure discriminative power over the labelled set **before** building: does selector presence separate `unsafe` from `safe`? Reuse `bench/backfill.py`'s RPC plumbing | Open |
