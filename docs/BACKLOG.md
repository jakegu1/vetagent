...
| W20 | Compress the snapshot archive before it outgrows the repo | **Rejected 2026-09-07 — the problem does not exist, and I filed it off a bad measurement.** I read `du -sh .git` (15 MB at 5 days) and the loose-object store, and projected ~250 MB/year of files and ~1 GB of history. Loose objects are a transient local artefact: git had not packed yet, and GitHub packs on receive. Packed, the whole repository is **4.10 MiB**, and the 9.33 MB of raw snapshot blob content collapses to **807 KB** — 20% of all packed blob content. Append-only NDJSON deltas almost perfectly, so four daily rewrites of a growing file cost little more than one. That is **~56 MiB/year**, against GitHub's 1 GB soft limit: roughly eighteen years of headroom. Nothing to fix, and gzip would have made the archive harder to read for a saving git was already making. Re-open only if `git count-objects -vH` reports `size-pack` above ~300 MiB. |
