...
| W10 | Create the two accounts the remaining directories need | **Everything submittable without an account of Jake's is submitted (2026-09-08).** Live: official registry, PulseMCP, Glama, Smithery. Queued: mcp.so, mcpservers.org, awesome-mcp-servers (their bot's Glama check answered), awesome-remote-mcp-servers, Docker MCP registry, Cline marketplace. What is left is two signups: a free **Console** account for the Claude plugin directory (the claude.ai form needs a Team/Enterprise org, which this account is not), and a **verified identity** on the OpenAI platform, which needs ID. `docs/OPENAI_SUBMISSION.md` has every other field written and its test cases already run against production | `CHANNELS` in `bench/scorecard.py`, updated only by someone who went and looked | Open |
