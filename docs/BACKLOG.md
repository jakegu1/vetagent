...
| W20 | Compress the snapshot archive before it outgrows the repo | 5 days of snapshots are 3.5 MB on disk and `.git` is already 15 MB, because each of four daily passes rewrites that day's whole NDJSON file and git keeps every version. Straight-lined that is ~250 MB/year of files and closer to 1 GB of history, against GitHub's 1 GB recommended limit -- so this bites in roughly a year, which is exactly when the archive finally becomes useful (W7 needs 60 days, and the death signal takes ~4 months to develop) | Gzip each day's file once the day is closed, or move the archive to release assets. Verify: `du -sh .git` growth per week falls by >5x, and `bench/build_dataset.py` still reads every historical day | Open |
