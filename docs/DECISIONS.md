...
| P4 | 每天采集新池快照 | 公开源只列活着的池子（实测 199 个样本采到 0 个 dead）；这份数据买不到也补不回来 | CI (`snapshot.yml` 每日定时) | 生效 |
...
3. ~~**P4**（每日快照）~~ → **已完成 2026-09-04**。`snapshot.yml` 每日 02:23 UTC 自动采集并提交；
   采不到数据时发 warning——「没有新数据」是异常而不是安静通过。
