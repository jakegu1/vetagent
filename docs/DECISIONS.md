...
| E19 | A finding may be **disclosed** without being **scored**, and an incomplete instrument must say so | Owner powers (pause, blacklist, mutable tax) are what the audit's adversarial `low` token relies on, and a caller is entitled to know a contract holds them. But measured over 417 contracts they do not separate bad from good at the cohort size available -- and mintable runs the wrong way. Scoring an unvalidated threshold is how the false positives got in; saying nothing hides a fact the caller needs. So it is reported and the verdict does not move. **Amended R13**: the scan finds only 31% of the powers the oracle asserts, so the R12 does-not-discriminate finding is withdrawn -- a hit is real, silence means nothing, and the payload now says which | `test_owner_powers` asserts the verdict is identical with and without the signal, and that finding nothing carries a caveat | Active |
